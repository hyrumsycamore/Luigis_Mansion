...
#define GPIO_OUT1_REG (DR_REG_GPIO_BASE + 0x010)
#define GPIO_OUT1_W1TS_REG (DR_REG_GPIO_BASE + 0x014)
#define GPIO_OUT1_W1TC_REG (DR_REG_GPIO_BASE + 0x018)
#define GPIO_ENABLE_REG (DR_REG_GPIO_BASE + 0x020)
...
#define GPIO_ACOU_NM1_INT1_REG (DR_REG_GPIO_BASE + 0x78)
#define GPIO_PCPU_INT1_REG (DR_REG_GPIO_BASE + 0x7C)
#define GPIO_PCPU_NMI_INT1_REG (DR_REG_GPIO_BASE + 0x80)
// Macros to get te address of GPIO_PINs and FUNC_IN and FUNC_OUT
#define GPIO_PIN_REG(n) ((DR_REG_GPIO_BASE + 0x80) + (0x04 * n))
#define GPIO_FUNC_IN_SEL_CFG_REG(n) ((DR_REG_GPIO_BASE + 0x130) + (0x04 * n))
#define GPIO_FUNC_OUT_SEL_CFG_REG(n) ((DR_REG_GPIO_BASE + 0x530) + (0x04 * n))

// GPIO REGISTER FEILDS 
#define GPIO_PAD_DRIVE 2
// IO MUX Registers
...
// Constants to work with the Pin bits
#define REG_BITS 32
#define MAX_PIN 39
// Default Settings
#define GPIO_FUNCOUT_SEL_DEFAULT 0x100
#define IO_MUX_REG_DEFAULT 0x2900

// Gets the value at the memory address of the pin 
#define REG(r) (*(volatile uint32_t *)(r))

// Macros for Setting, Clearing and Getting bits
#define REG_SET_BIT(r, b) (REG(r) = REG(r) | (0x01 << b))
#define REG_CLR_BIT(r, b) (REG(r) = REG(r) & ~(0x01 << b))
#define REG_GET_BIT(r, b) ((REG(r) & (0x01 << b)) != 0)
...
int32_t pin_reset(pin_num_t pin)
{
	...
	REG(GPIO_FUNC_OUT_SEL_CFG_REG(pin)) = GPIO_FUNCOUT_SEL_DEFAULT;
...
int32_t pin_pullup(pin_num_t pin, bool enable)
{
	...
	{ // hand-off work to RTC subsystem
		if (enable)
			return rtc_gpio_pullup_en(pin);
		else
			return rtc_gpio_pullup_dis(pin);
	}
...
int32_t pin_pulldown(pin_num_t pin, bool enable)
{
	...
	{ // hand-off work to RTC subsystem
		if (enable)
			return rtc_gpio_pulldown_en(pin);
		else
			return rtc_gpio_pulldown_dis(pin);
	}
...
{
	// Sanity check, makes sure that it's starting with a valid pin.
	if (pin < 0 || pin > MAX_PIN)
	{
		return -1;
	}

	// Checks wether or not the pin is in the ENABLE or ENABLE1
	if (pin < REG_BITS)
	{
		// Ser or clear the I/O pin bit in the ENABLE register
		if (enable)
		{
	...
	{
		// Set or clear the I/O pin bit in the ENABLE1 register
		if (enable)
		{
		...
		{
			REG_CLR_BIT((GPIO_ENABLE1_W1TC_REG), (pin - REG_BITS));
		}
	}
	return 0;
}
...
int32_t pin_odrain(pin_num_t pin, bool enable)
{
	...
	if (enable)
	{
		REG_SET_BIT((GPIO_PIN_REG(pin)), GPIO_PAD_DRIVE);
	}
	else
	{
		REG_CLR_BIT((GPIO_PIN_REG(pin)), GPIO_PAD_DRIVE);
	}
	return 0;
}
...
int32_t pin_set_level(pin_num_t pin, int32_t level)
{
	...
	// Checks wether or not the pin is in the OUT or OUT1 register
	if (pin < REG_BITS)
	{	
		// Set or clear the I/O pin bit in the OUT Register
		if (level)
		{
	...
	{
		// Set or clear the I/O pin bit in the OUT1 Register
		if (level)
		{
			REG_SET_BIT((GPIO_OUT1_W1TS_REG), (pin - REG_BITS));
		}
		else
		{
			REG_SET_BIT((GPIO_OUT1_W1TC_REG), (pin - REG_BITS));
		}
	}
	return 0;
}
...
int32_t pin_get_level(pin_num_t pin)
{
	...
	// Checks if the pin is the IN or IN1
	if (pin < REG_BITS)
	{
...
uint64_t pin_get_in_reg(void)
{
	...
	uint32_t R1 = REG(GPIO_IN1_REG);
	uint32_t R0 = REG(GPIO_IN_REG);
	uint64_t IN_REG = ((uint64_t)R1 << REG_BITS) | R0;
	return IN_REG;
}
...
uint64_t pin_get_out_reg(void)
{
	...
	uint32_t R1 = REG(GPIO_OUT1_REG);
	uint32_t R0 = REG(GPIO_OUT_REG);
	uint64_t OUT_REG = ((uint64_t)R1 << REG_BITS) | R0;
	return OUT_REG;
}
